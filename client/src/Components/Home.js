...
export default function Home() {
  ...
  useEffect(() => {
    if (!authtoken) {
      ...
      axios.post(`/blog/filter/${filter}`, { authtoken }).then(response => {
  ...
  const postLikeHandler = (id) => {
    axios.put('/blog/like', { id, authtoken }).then(response => {
  ...
  const postDislikeHandler = (id) => {
    axios.put('/blog/dislike', { id, authtoken }).then(response => {
      setChangeUI(!changeUI)
    }).catch(error => {
      toast.error(error.response.data.msg)
      setChangeUI(!changeUI)
  ...
  const postComment = (id) => {
    axios.post('/blog/addComment', { id, authtoken, comment })
      .then(response => {
